...
%function [predict, resids, chi2dof, model] = tsfit(data, model);
%
%or
%
%function predict = tsfit(time, model);
...
%      Construct the parameter list
    ...
    if ( model.n.ndeltav > 0 )
        ...
        for i = 1:model.n.ndeltav
            parlist(np+i,:) = padstr( sprintf('deltav_%8.3f', model.deltav(i)) );
        end
...
%  single component, and then ue that to make the matrix for all 3 comps.
    ...
    if ( model.n.ndeltav > 0 )
        ...
        %   Should be able to remove this for loop:
        for i = 1:model.n.ndeltav
            G0(:, ip+i) = (times - model.t.tdeltav(i)).*heaviside(times, model.t.tdeltav(i));
        end
