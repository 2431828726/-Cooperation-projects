...
nsep = nsep - max(polyval(p2,plotdata.time, [], mu2) - polyval(p1,plotdata.time, [], mu1));
hsep = hsep - max(polyval(p2,plotdata.time, [], mu2) - polyval(p1,plotdata.time, [], mu1));
...
pbaspect([1.33 1 1]);
orient portrait;
title(plotdata.sitename, 'FontSize', 18, 'Interpreter', 'none')
