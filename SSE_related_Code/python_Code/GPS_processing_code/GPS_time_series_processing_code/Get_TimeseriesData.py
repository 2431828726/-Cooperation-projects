...
if section == 5:
    ...
    sdir = '../GRACE+loading/seasonal/'  # Get the seasonal file in the local folder，step3
    stype = 'as'
...
#you can check the .ts file to see if the site has been written according to our requirements (e.g., for site 1000, it should include lon, lat, ...).
               ...
               # format in datafile is mm.
                ...
                dat2 = dat1.copy()
                dat2_NOAM = dat2.copy()#data for ITRF to NOAM 
...
#We need to use calc_geodvel to obtain the values of vrel_plate and vRPcov
                    ...
                    dat2_NOAM['east'][p] = dat2_NOAM['east'][p] - vrel_plate[0] * (dat1['time'][p] - dat1['time'][0])  # meter
                    dat2_NOAM['esig'][p] = np.sqrt(dat2_NOAM['esig'][p]**2 + vRPcov[0, 0] * ((dat1['time'][p] - dat1['time'][0]) ** 2))  # meter, due to error propagation law
                    dat2_NOAM['north'][p] = dat2_NOAM['north'][p] - vrel_plate[1] * (dat1['time'][p] - dat1['time'][0])  # meter
                    dat2_NOAM['nsig'][p] = np.sqrt(dat2_NOAM['nsig'][p]**2 + vRPcov[1, 1] * ((dat1['time'][p] - dat1['time'][0]) ** 2))  # meter, due to error propagation law
                    dat2_NOAM['height'][p] = dat2_NOAM['height'][p] - vrel_plate[2] * (dat1['time'][p] - dat1['time'][0])  # meter
                    dat2_NOAM['hsig'][p] = np.sqrt(dat2_NOAM['hsig'][p]**2 + vRPcov[2, 2] * ((dat1['time'][p] - dat1['time'][0]) ** 2))  # meter, due to error propagation law
